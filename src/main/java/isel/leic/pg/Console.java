...
public abstract class Console {
    ...
    /**
     * Value returned when no char is typed by
     */
	public static final char NO_CHAR = 0;
    /**
     * Value returned when no key is pressed by
     */
	...
	public static void open(String title, int lines, int cols) {
		...
		Console.lines = lines;
		Console.cols = cols;
	    view = new ConsoleView(title, lines, cols, "sprites.jpg");
	}
	private static void check() { if (view == null) open(DEFAULT_LINES, DEFAULT_COLS); }
	 ...
	 * @see #cursor(int, int)
	 */
	public static void print(String s) { check(); view.print(s); }
	 ...
	 * @see #cursor(int, int)
	 */
	public static void print(char c) { check(); view.printChar(c); }


	public static void print(Point higher, Point lower, int size) { check(); view.printSprite(higher, lower, size); }
	 ...
	 */
	public static void cursor(int lin, int col) { view.setCursor(lin, col); }
	 ...
	 * @see #color(int, int)
	 */
	public static void setBackground(int color) { view.setBackground(colors[color % colors.length]); }
	/**
	 * Change the colors of text for next writes
	 * @param color Foreground color
	 * @see #color(int, int)
	 */
	public static void setForeground(int color) { view.setForeground(colors[color % colors.length]); }
	 ...
	 * @see #getKeyPressed()
	 */
	public static boolean isKeyPressed(int code) { return view.isKeyPressed(code); }
	 ...
	 */
	public static boolean isKeyPressed() { return view.isAnyKeyPressed(); }
	 ...
	 */
	public static int getKeyPressed() { return view.getKeyPressed(); }
	...
	public static int waitKeyPressed(long timeout) {
		...
		if (timeout>0) timeout += System.currentTimeMillis();
		int k;
		while( (k=view.getKeyPressed())==NO_KEY && (timeout<=0 || System.currentTimeMillis() < timeout ));
		return k;
	}
...
*/
	...
	public static String nextLine(int field) {
		boolean oldEcho = view.isEcho();
		int lin = view.getLine(), col = view.getColumn();
		...
		while((c=waitChar(0))!='\n') {
		  if (c== KeyEvent.VK_ESCAPE) { res=null; break; } // Esc (27)
			...
			cursor(view.getLine(),view.getColumn()-1);
			print(' ');
			cursor(view.getLine(),view.getColumn()-1);
 		  }
